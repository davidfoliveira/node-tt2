					...
					do {
						...
						tcode = tcode.replace(fnTagRx,function(all,fn,args){
							...
							if ( TT.enabledFeatures == null || TT.enabledFeatures['CONDITIONS'] ) {
								if ( fn == "IF" ) {
									aux.inside.push("if");
									code._ += aux.indent+'tto._if ( '+_convertXpr(args)+",function(){ // <if>\n";
									aux.indent += "  ";
								}
								else if ( fn == "UNLESS" ) {
									aux.inside.push("if");
									code._ += aux.indent+'tto._if (!( '+_convertXpr(args)+") ) { // <if>\n";
									aux.indent += "  ";
								}
								else if ( fn == "ELSIF" ) {
//									aux.inside.push("if");
									aux.indent = aux.indent.substr(0,aux.indent.length-2);
									code._ += aux.indent+'}\n'+aux.indent+'else if ( '+_convertXpr(args)+" ) { // <if>\n";
									aux.indent += "  ";
								}
								else if ( fn == "ELSE" ) {
//									aux.inside.push("if");
									...
									aux.indent = aux.indent.substr(0,aux.indent.length-2);
									code._ += aux.indent+"}); // </if>\n"+aux.indent+"if ( tto._lastIfRV != null ) return tto._lastIfRV;\n";
									aux.inside.pop();
								}
								else
									keepGoing = true;
							...
							if ( keepGoing && (TT.enabledFeatures == null || TT.enabledFeatures['LOOPS']) ) {
								if ( fn == "FOREACH" && args.match(/^\s*(\w+)\s*in\s*([^\s]+)/i) ) {
									var
										scopeVar = RegExp.$1,
										runVar = RegExp.$2;
									code._ += aux.indent+"tto._foreach("+_convertXpr(runVar)+",function(i){ // <loop>\n";
									aux.indent += "  ";
							...
							if ( keepGoing && (TT.enabledFeatures == null || TT.enabledFeatures['BLOCKS']) ) {
								if ( fn.match(/BLOCK/i) ) {
									...
									var
										blockName = RegExp.$1,
										incArgs = _convertAssingList(RegExp.$2);
	...
	do {
		...
		xpr = xpr.replace(xprRx,function(a,opar,str1,str2,expr,cpar){
			...
			if ( str1 != null )
				...
				parts.push("tto.getvar('"+prop+"')."+fn+".apply(tto.getvar('"+prop+"'),["+args+"])");
			}
